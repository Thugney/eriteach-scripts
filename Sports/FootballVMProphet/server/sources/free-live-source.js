...
export function fetchFreeLive(rootDir) {
  ...
  const health = { ok: false, source: endpoint || 'not-configured', fetchedAt: null, cacheUsed: false, warnings: [] };

  if (!endpoint) {
    health.warnings.push('FREE_LIVE_ENDPOINT is not configured. Generic custom live endpoint is disabled.');
  ...
  try {
    ...
    const payload = { matches: Array.isArray(parsed.matches) ? parsed.matches : [], fetchedAt: new Date().toISOString(), source: endpoint };
    ...
    if (cached) {
      ...
      health.warnings.push(`Showing cached custom live data from ${cached.fetchedAt}. Treat as stale.`);
